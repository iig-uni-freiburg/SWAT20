...
public abstract class AbstractLogModelContainer extends AbstractComponentContainer<LogModel> implements ComponentListener<LogModel> {
        ...
        public AnalysisContainer getContainerAnalysis(String logModelName) throws ProjectComponentException {
                return getContainerAnalysis(logModelName, true);
        }

        public AnalysisContainer getContainerAnalysis(String logModelName, boolean validateLogName) throws ProjectComponentException {
                if (validateLogName) {
        ...
        public void renameComponent(String oldName, String newName, boolean notifyListeners) throws ProjectComponentException {
                /* 
                 * copy file, since logs are not written to memory and therefore 
                 * can't be written out of memory after deleting the old 
                 * component file.
                 */
                final File oldFileReference = getComponent(oldName).getFileReference();
                final File tempFile = new File(new File(oldFileReference.getParent()).getParent(), oldFileReference.getName());
                try {
                        Files.copy(oldFileReference.toPath(), tempFile.toPath(), StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.COPY_ATTRIBUTES, LinkOption.NOFOLLOW_LINKS);
                } catch (IOException ex) {
                        throw new ProjectComponentException(ex);
                }
                super.renameComponent(oldName, newName, notifyListeners);
                final File newFileReference = getComponentFile(newName);
                getComponent(newName).setFileReference(newFileReference);
                try {
                        Files.move(tempFile.toPath(), newFileReference.toPath(), StandardCopyOption.REPLACE_EXISTING);
                } catch (IOException ex) {
                        throw new ProjectComponentException(ex);
                }

                // rename reference in all views of this log
                for (LogView view : getComponent(newName).getLogViews()) {
                        view.setParentLogName(newName);
                        SwatComponents.getInstance().getContainerLogViews().storeComponent(view.getName());
                }
        }
        ...
        public void componentRenamed(LogModel component, String oldName, String newName) throws ProjectComponentException {
                if (analysisContainers.containsKey(oldName)) {
                        Collection<Analysis> analyses = getContainerAnalysis(oldName, false).getComponents();
                        getContainerAnalysis(oldName, false).removeComponents(true, false);
...
//		}
        ...
        /**
         * Add log file, copy file reference into new file, store new file under
         * given nam
         *
         * @param logFile
         * @param name
