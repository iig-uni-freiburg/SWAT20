package de.uni.freiburg.iig.telematik.swat.ext;
 ...
 * component that was added with a constraint that matches the Leaf's name. Extra space is distributed among row/column siblings according to their 0.0 to 1.0 weight. If no weights are specified then the last sibling always gets all of the extra space, or space reduction.
 * 
 * <p>
...
public class MultiSplitLayout implements LayoutManager {
	...
	/**
	 * Add a component to this MultiSplitLayout. The <code>name</code> should match the name property of the Leaf node that represents the bounds of <code>child</code>. After layoutContainer() recomputes the bounds of all of the nodes in the model, it will set this child's bounds to the bounds of the Leaf node with <code>name</code>. Note: if a component was already added with the same name, this
	...
	private void layoutShrink(Split split, Rectangle bounds) {
		...
		if (split.isRowLayout()) {
			...
			while (splitChildren.hasNext()) {
				...
				double splitChildWeight = (onlyShrinkWeightedComponents) ? splitChild.getWeight() : (splitChildBounds.getWidth() / (double) totalWidth);
			...
			while (splitChildren.hasNext()) {
				...
				double splitChildWeight = (onlyShrinkWeightedComponents) ? splitChild.getWeight() : (splitChildBounds.getHeight() / (double) totalHeight);
		...
		/*
		 * The bounds of the Split node root are set to be big enough to contain all of its children. Since Leaf children can't be reduced below their (corresponding java.awt.Component) minimum sizes, the size of the Split's bounds maybe be larger than the bounds we were asked to fit within.
		 */
	...
	private void layoutGrow(Split split, Rectangle bounds) {
		...
		/*
		 * Layout the Split's child Nodes' along the X axis. The bounds of each child will have the same y coordinate 
		 * and height as the layoutGrow() bounds argument. Extra width is allocated to the to each child with a 
		 * non-zero weight: newWidth = currentWidth + (extraWidth * splitChild.getWeight()) Any extraWidth 
		 * "left over" (that's availableWidth in the loop below) is given to the last child.
		 * 
		 * Note that Dividers always have a weight of zero, and they're never the last child.
		 */
		if (split.isRowLayout()) {
			double x = bounds.getX();
		...
		/*
		 * Layout the Split's child Nodes' along the Y axis. The bounds of each child will have the same x coordinate and width as the layoutGrow() bounds argument. Extra height is allocated to the to each child with a non-zero weight: newHeight = currentHeight + (extraHeight * splitChild.getWeight()) Any extraHeight "left over" (that's availableHeight in the loop below) is given to the last
		 * child. Note that Dividers always have a weight of zero, and they're never the last child.
		 */
		else {
			double y = bounds.getY();
	...
	/*
	 * Second pass of the layout algorithm: branch to layoutGrow/Shrink as needed.
	 */
	 ...
	 * 
	 * If the Dividers are "floating" then set the bounds of each node to accomodate the preferred size of all of the Leaf's java.awt.Components. Otherwise, just set the bounds of each Leaf/Split node so that it's to the left of (for Split.isRowLayout() Split children) or directly above the Divider that follows.
	 * 
	 * This pass sets the bounds of each Node in the layout model. It does not resize any of the parent Container's (java.awt.Component) children. That's done in the second pass, see layoutGrow() and layoutShrink().
	 */
	private void layout1(Node root, Rectangle bounds) {
		if (root instanceof Leaf) {
			...
			/*
			 * Layout the Split's child Nodes' along the X axis. The bounds of each child will have the same y coordinate and height as the layout1() bounds argument.
			 * 
			 * Note: the column layout code - that's the "else" clause below this if, is identical to the X axis (rowLayout) code below.
			 */
			if (split.isRowLayout()) {
				double x = bounds.getX();
				while (splitChildren.hasNext()) {
			...
			/*
			 * Layout the Split's child Nodes' along the Y axis. The bounds of each child will have the same x coordinate and width as the layout1() bounds argument. The algorithm is identical to what's explained above, for the X axis case.
			 */
			else {
				double y = bounds.getY();
				while (splitChildren.hasNext()) {
			...
			/*
			 * The bounds of the Split node root are set to be just big enough to contain all of its children, but only along the axis it's allocating space on. That's X for rows, Y for columns. The second pass of the layout algorithm - see layoutShrink()/layoutGrow() allocates extra space.
			 */
			minimizeSplitBounds(split, bounds);
		}
	}
	...
	private void checkLayout(Node root) {
		if (root instanceof Split) {
			...
			if (split.getChildren().size() <= 2) {
				// System.out.println(ListUtils.toString(split.getChildren(), ','));
	...
	public static abstract class Node {
		...
		/**
		 * Set the bounding Rectangle for this node. The value of bounds may not be null. The default value of bounds is equal to <code>new Rectangle(0,0,0,0)</code>.
		 * 
		 * @param bounds
