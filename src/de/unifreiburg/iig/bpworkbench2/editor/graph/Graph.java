...
public class Graph extends mxGraph {
		...
		if (edgeSource != null && edgeTarget != null) {
			if (edgeSource.getClass() == edgeTarget.getClass()) {
				...
				if (edgeTarget instanceof mxTransition && isDNDgenerated) {
					...
					try {
						nodeName = addPNNode(edgeTarget, netContainer, prefix);
	 ...
	 * @param nodeName
	 */
	public void addGraphicalInfoToPN(mxCell cell, AbstractGraphicalPN<?, ?, ?, ?, ?, ?, ?> n, String nodeName) {
		...
		try {
			nodeGraphics = new NodeGraphics(position, dimension, fill, line);
		} catch (ParameterException e) {
			e.printStackTrace();
		}
		...
		if (cell.getId().startsWith("arc")) {
			...
			try {
				arcGraphics = new ArcGraphics(positions, new Line());
			} catch (ParameterException e) {
				e.printStackTrace();
			}
	...
	// * cell. This implementation uses <convertValueToString> if
	// <labelsVisible>
	...
	// AbstractGraphicalPN<?, ?, ?, ?, ?, ?, ?> n = (AbstractGraphicalPN<?, ?,
	// ?, ?, ?, ?, ?>) ((mxCell) cell)
	// .getValue();
	...
	public void cellLabelChanged(Object cell, Object value, boolean autoSize) {
		...
		try {
			...
			// AnnotationGraphics annotation = new AnnotationGraphics(new
			// Offset(offset.getX(), offset.getY()), new Fill(),new Line(),new
			// Font());
			// Map<String, AnnotationGraphics> placeLabelAnnotationGraphics =
			// null;
			// if(n.getPetriNetGraphics().getPlaceLabelAnnotationGraphics() ==
			// null)
			// placeLabelAnnotationGraphics = new HashMap<String,
			// AnnotationGraphics>() ;
			// else placeLabelAnnotationGraphics =
			// n.getPetriNetGraphics().getPlaceLabelAnnotationGraphics();
			//
	...
	public String getLabelPosition(Object cell) {
		...
		if (cell != null) {
			...
			// (AbstractGraphicalPN<?, ?, ?, ?, ?, ?, ?>) ((mxCell) cell)
			// .getValue();
			// if (n.getPetriNet().getPlace(((mxCell) cell).getId()) != null) {
			// result = "";
			// getModel().beginUpdate();
			...
			// getModel().endUpdate();
			// }
			// if (n.getPetriNet().getTransition(((mxCell) cell).getId()) !=
			// null) {
			//
			// result = "";
			// getModel().beginUpdate();
	...
	public void updatePositionInPN(mxCell cell, NodeGraphics pG) {
		pG.getPosition().setX(cell.getGeometry().getX());
		pG.getPosition().setY(cell.getGeometry().getY());
	}

	private void addNodeToMap(Map<Integer, String> a, String string, String prefix) {
		if (string.startsWith(prefix) && isInteger(string.substring(prefix.length()))) {
	...
	public void addTPArcToPN(Object edge, mxCell edgeSource, mxCell edgeTarget, AbstractGraphicalPN<?, ?, ?, ?, ?, ?, ?> n) {
		try {
	...
	public void addPTArcToPN(mxCell edge, mxCell edgeSource, mxCell edgeTarget, AbstractGraphicalPN<?, ?, ?, ?, ?, ?, ?> n) {
		try {
	...
	public void addLabelToNode(mxCell edgeTarget, AbstractGraphicalPN<?, ?, ?, ?, ?, ?, ?> n, String nodeName) {
	...
	public int getNodeName(mxCell edgeTarget, String prefix, SortedMap<Integer, String> sortedMap) {
