...
public class GraphComponent extends mxGraphComponent {
	...
	public GraphComponent(Graph graph) {
					...
					if (cell instanceof mxPlace) {
						...
						if (marks != null) {
							...
							if (n.getPetriNet().getInitialMarking() == null) {
								...
								initialMarking = (PTMarking) n.getPetriNet().getInitialMarking();
							try {
		...
		// ?, ?, ?, ?, ?, ?>) value;
		// CPNPlace place = (CPNPlace)
		// n.getPetriNet().getPlace(((mxCell)cell).getId());
		//
		// try {
		...
		// }
		// try {
		// System.out.println(place.getState().multiplicity("black")+
		// "##############1");
		// int multiplicity = place.getState().decMultiplicity(new
		// String("black"));
		// // ((AbstractCPNPlace<CPNFlowRelation>)
		// place).getState().setMultiplicity("black", 1);
		// System.out.println(place.getState().multiplicity("black")+
		// "##############2");
